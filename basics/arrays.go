...
func main() {
	...
	fmt.Println(*copiedArray)
	// Here we are dereferencing the pointer, but this is irrelevant actually, because we don't need to dereference the pointer because it's not a variable. It is not an array variable. It is a pointer variable which contains pointer to an array which has three elements, and all those elements are integer type.
	fmt.Println(orignalArray)
